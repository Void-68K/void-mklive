...
register_binfmt() {
    ...
    case "${_targetarch}" in
        ...
        mipsel)
            ...
            ;;
        m68k)
            if [ "$_hostarch" = "m68k" ] ; then
                return
            fi
            _cpu=m68k
            _magic="\x7fELF\x01\x02\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x00\x04"
            _mask="\xff\xff\xff\xff\xff\xff\xfe\x00\xff\xff\xff\xff\xff\xff\xff\xff\xff\xfe\xff\xff"
            ;;
        x86_64)
            _cpu=x86_64
